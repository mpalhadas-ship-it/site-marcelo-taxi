...
const PRECO_KM_FIXO = 4;
const PRECO_KM_API = 4;

const cidadesFixas = [
  { nomes: ["santos dumont"], km: 43 },
  { nomes: ["lima duarte"], km: 60 },
  { nomes: ["ubá", "uba"], km: 100 },
  ...
  { nomes: ["santos dumont aeroporto", "aeroporto santos dumont"], km: 200 },
...
function calcularComKm(km, precoKm) {
  const valorTotal = arredondarParaMultiploDe10(km * precoKm);
...
async function buscarDistanciaAPI(destino) {
  ...
  // Usa um proxy CORS para contornar restrição do browser
...
function buscarTempo() {
  ...
  debounceTimer = setTimeout(function () {
    ...
    if (cidadeFixa && !cidadeFixa.especial) {
      const { valorTotal, valorVista, parcelaDisplay } = calcularComKm(cidadeFixa.km, PRECO_KM_FIXO);
...
async function exibirValor() {
  ...
  if (cidadeFixa) {
    let valorTotal, valorVista, parcelaDisplay;

    if (cidadeFixa.especial) {
      valorVista = cidadeFixa.especial.avista;
      valorTotal = cidadeFixa.especial.parcelado;
      const valorParcela = cidadeFixa.especial.parcela;
      parcelaDisplay = `R$ ${valorParcela.toFixed(2).replace('.', ',')}`;
    } else {
      ({ valorTotal, valorVista, parcelaDisplay } = calcularComKm(cidadeFixa.km, PRECO_KM_FIXO));
    }

    resultadoDiv.innerHTML = montarResultadoHTML(valorTotal, valorVista, parcelaDisplay);

  } else {
    // Cidade não cadastrada — consulta Google Maps API
    ...
    try {
      ...
      if (km && km > 0) {
        const { valorTotal, valorVista, parcelaDisplay } = calcularComKm(km, PRECO_KM_API);
        resultadoDiv.innerHTML = montarResultadoHTML(valorTotal, valorVista, parcelaDisplay);
      } else {
